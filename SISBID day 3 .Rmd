...
dat %>% filter(`Passed QC`==1 & Center=='BCM') %>%
    group_by(Population) %>%
  summarize(avg_align=mean(`Aligned Non Duplicated Coverage`,na.rm=TRUE))
```

```{r}
#lm(`Total Sequence`~Center,data=dat)
...
.(name,mean(prop)),name]
baby_dt[sex=="F",
.(name,aveprop=mean(prop)),name]# group by name

```
Re-assign
```{r}
baby_dt[,aveprop:=mean(prop),name] #automatically updates the data baby_dt
```
